(function ($) {
    Drupal.behaviors.turism1_23 = {
        attach: function (context, settings) {
            ...
            // 🔁 Forțează inputul CAP2_R_CC să fie de tip text
            jQuery('input.input-country', context).attr('type', 'text');

            // 🧠 Atașează validarea codului de țară la input și change
...
function changeSelectCountry(elem) {
    ...
    $select.find('option').each(function (index, opt) {
        if (opt.value.toLowerCase() === codIntrodus.toLowerCase()) {
    ...
    if ($msgContainer.length === 0) {
        ...
        $row.find('td:nth-child(3)').append($msgContainer);
    }
