(function ($) {
    Drupal.behaviors.turism1_23 = {
        attach: function (context, settings) {
            ...
            // 🆕 Atașează funcția de validare cod țară
            jQuery('input.input-country', context).once('input-country-change').on('change', function () {
...
function changeSelectCountry(elem) {
    ...
    // Găsim sau creăm container pentru mesaj
    var $msgContainer = $row.find('.country-error-message');
    if ($msgContainer.length === 0) {
        $msgContainer = $('<div class="country-error-message" style="color:red; font-size:12px; margin-top:2px;"></div>');
        $row.find('td:nth-child(3)').append($msgContainer); // Adaugă sub coloana cod
    }

    if (!gasit) {
        jQuery(elem).addClass('invalid-country');
        $select.prop('selectedIndex', 0);
        $msgContainer.text("Nu există așa cod în clasificator");
    } else {
        jQuery(elem).removeClass('invalid-country');
        $msgContainer.text(""); // Șterge mesajul
