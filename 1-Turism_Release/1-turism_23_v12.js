...
function changeSelectCountry(elem) {
    ...
    console.log("Cod introdus (formatat):", codIntrodus);
    ...
    // Găsim sau creăm container pentru mesaj direct în celula inputului
    var $inputCell = jQuery(elem).closest('td');
    var $msgContainer = $inputCell.find('.country-error-message');

    if ($msgContainer.length === 0) {
        $msgContainer = $('<div class="country-error-message" style="color:red; font-size:12px; margin-top:2px;"></div>');
        $inputCell.append($msgContainer);
    }
