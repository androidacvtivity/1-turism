...
function changeSelectCountry(elem) {
    var codIntrodus = jQuery(elem).val();
    ...
    $select.find('option').each(function (index, opt) {
        if (opt.value === codIntrodus) {
            ...
            gasit = true;
            return false; // ieșim din loop
        }
    });

    if (!gasit) {
        $select.prop('selectedIndex', 0); // resetăm dacă nu e găsit
    }

    // Trigger manual funcția care actualizează codul (opțional)
